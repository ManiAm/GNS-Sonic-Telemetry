...
Examining this workflow provides a clear understanding of the fundamental RPC mechanics: method invocation, parameter serialization, network transmission, remote execution, and response handling. Once these core ideas are understood through JSON-RPC, it becomes easier to appreciate how more advanced frameworks such as [gRPC](./04_README_gRPC.md) build on the same RPC principles while adding stronger typing, better performance, and support for advanced communication patterns such as streaming. The next document covers [Protocol Buffers](./03_README_proto.md), the compact binary serialization format that gRPC uses for encoding data.
